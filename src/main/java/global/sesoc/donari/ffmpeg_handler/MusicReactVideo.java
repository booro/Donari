...
public class MusicReactVideo {
	...
	public void makeBasicVid(String inputImgsPath, String outputVidPath, String musicPath, String serverPath){
		String COPIED_IMAGES = serverPath+"copied_images/";
		String PROCESSING_VID = serverPath+"processing_vid/";
		String PROCESSED_VIDS = serverPath+"processed_vids/";
		...
		File outputVidFile = new File(new File(outputVidPath).getParent());
		
		if(!outputVidFile.exists()) {
			outputVidFile.mkdirs();
		}
		...
		for (int i = 0; i < imgs.length; i++) {
			makeVideo.reformatImg(imgs[i].getPath(), COPIED_IMAGES+String.format("%03d", i)+".jpg", "1280", "960");
		}
		...
		try {
			...
			for (int i = 0; i < beats.length; i++) {
				if (imageCount >= processedIimgs.length) {
					txt += "duration 00:00:00.487";
					break;
				}
				if (beats[i].energy > 0.27) {
